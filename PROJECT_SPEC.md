...
> **状态声明(2026-06):本系统已实现并仅在本机运行,确定不上 VPS。** 下文凡涉及「迁移 VPS / 上传到服务器存储 / 反代鉴权」的设计,均为**保留的可选能力**而非目标。其中三条架构原则(`DATA_DIR` 环境变量、`file_path` 只存引用、应用内不做登录)即便永不上 VPS 也是好设计,予以保留。当前进度与后续版本规划见 [`ROADMAP.md`](./ROADMAP.md)。
...
单人使用。**仅本机运行(不上 VPS);架构上保留迁移就绪能力但不作为目标,见上「状态声明」。**
